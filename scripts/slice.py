...
def read_audio(path: str, sampling_rate: int = 16000) -> torch.Tensor:
    # librosa.load で読み込み＆指定サンプリングレートにリサンプリング
    # mono=True なのでマルチチャネルの場合は自動で平均化されて 1ch になります
    wav, _ = librosa.load(Path(path), sr=sampling_rate, mono=True)

    # numpy -> torch.Tensor (元の関数同様 1 次元テンソルを返す)
...
if __name__ == "__main__":
    ...
    # スレッドローカルストレージにモデルを保持する
    thread_local = threading.local()

    def get_vad_model():
        if not hasattr(thread_local, "vad_model"):
            thread_local.vad_model, thread_local.utils = torch.hub.load(
                repo_or_dir="litagin02/silero-vad",
                model="silero_vad",
                onnx=True,
                trust_repo=True,
            )
        return thread_local.vad_model, thread_local.utils
    ...
    def process_file(file: Path):
        try:
            ...
            # エラー時は例外を再送出せず、呼び出し元でハンドリングできるようにする、
            # あるいはここでログを出して (0, 0) を返す
            raise e
    ...
    with ThreadPoolExecutor(max_workers=num_processes) as executor:
        # Future と file の対応を保持する辞書
        future_to_file = {
            executor.submit(process_file, file): file for file in audio_files
        }

        # tqdm で進捗表示
        with tqdm(total=len(audio_files), file=SAFE_STDOUT, dynamic_ncols=True) as pbar:
            for future in as_completed(future_to_file):
                file = future_to_file[future]
                try:
                    time_sec, count = future.result()
                    total_sec += time_sec
                    total_count += count
                except Exception as e:
                    error_files.append((file, e))
                finally:
                    pbar.update(1)
