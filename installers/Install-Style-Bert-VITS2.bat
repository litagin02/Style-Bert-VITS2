...
@REM FFmpegのチェックとダウンロード
echo --------------------------------------------------
echo Checking FFmpeg Installation...
echo --------------------------------------------------
echo Executing: ffmpeg -version
ffmpeg -version >nul 2>&1
if !errorlevel! neq 0 (
	if not exist "lib\ffmpeg\bin\ffmpeg.exe" (
		echo --------------------------------------------------
		echo FFmpeg is not installed, so download and use it.
		echo Downloading FFmpeg...
		echo --------------------------------------------------
		set FFMPEG_URL=https://www.gyan.dev/ffmpeg/builds/ffmpeg-release-essentials.zip
		set FFMPEG_DST=%~dp0lib\ffmpeg.zip
		
		echo Executing: curl -L !FFMPEG_URL! -o "!FFMPEG_DST!"
		curl -L !FFMPEG_URL! -o "!FFMPEG_DST!"
		if !errorlevel! neq 0 ( pause & popd & exit /b !errorlevel! )

		echo --------------------------------------------------
		echo Extracting FFmpeg...
		echo --------------------------------------------------
		@REM PowerShellを使って解凍 (7zがない場合も考慮)
		echo Executing: %PS_CMD% Expand-Archive -Path "!FFMPEG_DST!" -DestinationPath "%~dp0lib\ffmpeg_tmp" -Force
		%PS_CMD% "Expand-Archive -Path '!FFMPEG_DST!' -DestinationPath '%~dp0lib\ffmpeg_tmp' -Force"
		if !errorlevel! neq 0 ( pause & popd & exit /b !errorlevel! )

		@REM 解凍したフォルダの中身を移動 (フォルダ名がバージョンによって変わるため)
		for /d %%D in ("%~dp0lib\ffmpeg_tmp\*") do (
			move "%%D" "%~dp0lib\ffmpeg"
		)
		rmdir "%~dp0lib\ffmpeg_tmp"
		del "!FFMPEG_DST!"
		
		echo FFmpeg installed to lib\ffmpeg
	) else (
		echo FFmpeg found in lib\ffmpeg
	)
)
...
echo ----------------------------------------
echo Executing: python scripts\server_editor.py --inbrowser
python scripts\server_editor.py --inbrowser
pause
